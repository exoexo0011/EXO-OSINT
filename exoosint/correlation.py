...
# ---------------------------------------------------------------------------
# Lightweight verifications used during correlation. These are kept minimal
# (sub-second per call where possible) to keep correlation runs fast.
# ---------------------------------------------------------------------------
...
def _username_quick_check(username: str, timeout: int, threads: int = 8) -> List[Dict[str, Any]]:
    """Check a username against a small set of high-signal platforms."""
    quick = {
...
def _derive_from_domain(domain: str) -> List[Tuple[str, str]]:
    """domain -> common candidate emails."""
...
def _derive_from_ip(ip: str, target_report: TargetReport) -> List[Tuple[str, str]]:
    """ip -> reverse DNS -> domain candidate."""
    ...
    for m in target_report.modules:
        ...
        # also consider geo-derived nothing useful
    return out


def _derive_from_phone(phone: str) -> List[Tuple[str, str]]:
    """phone -> nothing reliably auto-checkable; correlation surfaces dorks instead."""
    return []
...
def correlate(
    ...
    """For each target report, derive related identifiers, run lightweight
    confirmation checks, and append a `correlation` ModuleResult to that target.

    Returns the list of ModuleResults in the same order. Mutates each
    TargetReport by appending the correlation module + correlation links.
    """
    out_results: List[ModuleResult] = []

    # Index already-investigated identifiers so we don't re-check them
    ...
    for tr in target_reports:
        ...
        # Dedupe and skip already-known identifiers
        ...
        # Run lightweight checks per candidate
            ...
            try:
                if ctype == "email":
                    ...
                    link = CorrelationLink(
                        ...
                        confirmed=resolves,
                        source="dns" if resolves else "derivation",
                        note="domain resolves" if resolves else "candidate domain",
                    )
                    tr.correlations.append(link)
                    mod.add(f"corr_domain_{cval}", cval,
                            severity="medium" if resolves else "info",
                            source="correlation", note=link.note)
                    ...
                    link = CorrelationLink(
                        ...
                        confirmed=confirmed,
                        source="username_quick_check",
                        note=(f"matched on {len(found)} platform(s): "
                              + ", ".join(h["platform"] for h in found)) if found else "no quick matches",
                    )
                    tr.correlations.append(link)
                    mod.add(
                        f"corr_username_{cval}",
                        cval,
                        severity=("high" if len(found) >= 2 else "medium") if confirmed else "info",
                        source="correlation",
                        note=link.note,
                    )
                    for h in found:
                        mod.add(f"corr_profile_{h['platform']}", h["url"],
                                severity="medium", source="correlation",
                                note=f"derived-username match for {cval}",
                                profile_url=h["url"])
        ...
        mod.summary = (
            f"{len(unique)} candidates derived | "
            f"{confirmed_count} confirmed via lightweight check"
        )
