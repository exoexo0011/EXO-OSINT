...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--modules", default="all", help="Comma-separated modules to run (default: all relevant)")
    p.add_argument("--username-platforms", default="", help="Comma-separated platforms to check for username")
    p.add_argument("--report", default="", help="Report formats: html, json, csv")
    ...
    p.add_argument("--country", default="IN", help="Default country for phone parsing")
    p.add_argument("--region", default=None, help="Alias for --country")
    p.add_argument("--depth", type=int, choices=[1, 2, 3], default=2, help="Investigation depth")
    p.add_argument("--stealth", action="store_true", help="Enable stealth mode")
    p.add_argument("--investigate", action="store_true", help="Run ALL modules")
    
    # ← NEW DATING FLAGS
    p.add_argument("--dating", action="store_true", help="Run dating & matrimonial app enumeration")
    p.add_argument("--dating-categories", nargs="+", choices=["dating", "indian", "teen", "social", "chat"], 
                   help="Limit dating check to categories")

    p.add_argument("--no-correlation", action="store_true", help="Disable correlation engine")
    return p
    ...
    try:
        ...
        if ttype == "ip":
            if run_all or "ip" in modules:
                tr.modules.append(ip_mod.run(target, timeout=args.timeout, threads=args.threads, depth=args.depth))
        elif ttype == "domain":
            if run_all or "domain" in modules:
                tr.modules.append(domain_mod.run(target, timeout=args.timeout, threads=args.threads, depth=args.depth))
        elif ttype == "email":
            if run_all or "email" in modules:
                tr.modules.append(email_mod.run(target, timeout=args.timeout, threads=args.threads, run_domain_recon=(run_all or "domain" in modules), depth=args.depth))
        elif ttype == "username":
            if run_all or "username" in modules:
                platforms = [p.strip() for p in (args.username_platforms or "").split(",") if p.strip()]
                tr.modules.append(username_mod.run(target, timeout=args.timeout, threads=args.threads, platforms=platforms or None, depth=args.depth))
        elif ttype == "phone":
            if run_all or "phone" in modules:
                tr.modules.append(phone_mod.run(target, default_region=region, timeout=args.timeout, depth=args.depth))

        # ==================== DATING RECON ====================
        if (args.dating or args.investigate or run_all) and ttype in ("email", "phone"):
            ui.section("DATING & MATRIMONIAL RECON")
            dating_results = dating_recon.run_dating_recon(
                {"emails": [target] if ttype == "email" else [], 
                 "phones": [target] if ttype == "phone" else []},
                {"proxies": None, "stealth": args.stealth}
            )
            tr.modules.append(dating_results)
...
# Rest of your file remains the same (main function etc.)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # Correlation
    want_correlation = (args.investigate and not args.no_correlation) or ("correlation" in [m.strip().lower() for m in args.modules.split(",")])
    if want_correlation and investigation.targets:
        ui.section("CORRELATION ENGINE")
        try:
            corr_mod.correlate(investigation.targets, timeout=min(args.timeout, 8), threads=max(args.threads // 2, 6))
        except Exception as exc:
